...
10. <get_file_context path="文件绝对路径" intent="任务简述" /> — 获取指定文件的函数级摘要和历史 Bug 问题单。
    ...
    返回 [FILE_SUMMARY] 和 [BUG_ALERT] 两个区块，帮助你在阅读或修改文件前了解其结构和历史问题。

## 渐进式文件上下文
当需要阅读或修改一个程序文件时，建议先通过 `get_file_context` 获取该文件的函数级摘要和历史 Bug 问题单。
- 函数摘要帮助你了解文件结构，决定是阅读完整文件还是仅阅读特定行范围
- 历史 Bug 问题单作为警示，帮助你避免重复踩坑
对于简单场景（如仅需查看文件的一小段），可以跳过此步骤直接使用 file_view。
注意：同一个文件的函数级摘要和 Bug 问题单，在同一会话中只需召回一次，不要重复调用 `get_file_context`。
