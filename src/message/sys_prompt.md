...
3. `<str_replace path="{项目代码目录}/文件路径" summary="一句话摘要（不超过50字符）"><old>旧代码</old><new>新代码</new></str_replace>` — 修改现有文件。**必须提供 `summary` 属性**，简要说明本次修改。系统将优先返回该摘要；若未提供摘要（向后兼容），则自动生成包含替换详情的摘要（如“文件已修改：{path}，替换了 1 处（{old_len} → {new_len} 字符）”）。
