...
def test_build_filename_with_port():
    """域名包含端口号"""
    url = "http://localhost:8080/test"
    filename = build_filename(url)
    assert filename.startswith("localhost_8080_")
    assert filename.endswith(".json")


# ========== parse_content ==========
def test_parse_content_title():
    """正常路径：提取页面标题"""
...
def test_parse_content_title_stripped():
    """标题前后有空白字符"""
    html = "<html><head><title>  Hello World  </title></head><body></body></html>"
    result = parse_content(html)
    assert result["page_title"] == "Hello World"
...
def test_parse_content_empty_title():
    """title 标签存在但内容为空"""
...
def test_parse_content_links():
    """正常路径：提取所有带 href 的 <a> 标签"""
    html = """
    <html><body>
...
def test_parse_content_headings():
    """正常路径：提取 h1~h3 标题"""
    html = """
    <html><body>
        <h1>Main Title</h1>
...
def test_parse_content_empty_headings():
    """边界条件：空 h1 标签不被收录"""
    html = """
    <html><body>
        <h1></h1>
...
def test_parse_content_empty_link_text():
    """边界条件：<a> 标签的 text 为空时记录空字符串"""
    html = '<html><body><a href="/only-href"></a></body></html>'
    result = parse_content(html)
    assert len(result["links"]) == 1
    assert result["links"][0] == {"text": "", "href": "/only-href"}
...
def test_crawl_success(mock_get):
    ...
    output_dir = tempfile.mkdtemp()
    try:
        result_path = crawl("https://example.com", output_dir)
        assert result_path.exists()
        with open(result_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        assert data["page_title"] == "E2E"
        assert len(data["links"]) == 1
        assert data["links"][0]["text"] == "Link A"
    finally:
        import shutil
