"""Memory5Adapter —— 将 Memory5 适配到 query_loop 所需的 MemoryBackend 接口

用法：替代 Memory1Adapter，让 query_loop 通过统一的 MemoryBackend
接口使用 memory5 的三层记忆（TF-IDF + 余弦相似度）。
"""
...
from src.memory_ex.memory5.memory5 import Memory5
from src.utility.config_loader import global_cfg

logger = logging.getLogger(__name__)


class Memory5Adapter(MemoryBackend):
    """适配 Memory5（TF-IDF 记忆）到 query_loop 的统一接口。"""

    def __init__(self, config: Optional[dict] = None):
        # 1. 合并 memory 配置：先取传入的 config，再补 global_cfg 的通用字段
        merged_cfg = dict(config) if config else {}

        # 用 global_cfg.memory 的 storage_path 作为 storage_dir（如果适配器 config 未提供）
        if "storage_dir" not in merged_cfg:
            merged_cfg["storage_dir"] = getattr(
                global_cfg.memory, "storage_path", ".memdir"
            )

        # 用 global_cfg.memory 的容量参数（如果适配器 config 未提供）
        ...
        self._mgr = Memory5(config=merged_cfg)

    # ------------------------------------------------------------------
    #  实现 MemoryBackend 接口（与 Memory1Adapter 相同的方法签名）
    # ------------------------------------------------------------------
    ...
    def inject_context(self, current_query: str) -> str:
        ...
        for r in results:
            ...
            ts = r.get('created_at', r.get('accessed_at', ''))
    ...
    def persist_and_maintain(self) -> None:
        """会话结束时：晋升工作记忆 → 压缩短期记忆 → 遗忘过期记忆"""
        promoted = self._mgr.promote_all_working()
        if promoted:
            logger.info(f"已将 {promoted} 条工作记忆晋升为短期记忆")
