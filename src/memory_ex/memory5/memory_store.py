"""memory5 独立存储层 —— JSON 文件持久化 + 原子写入 + 滚动备份 + 损坏恢复

从 src/memory/memory_store.py 重新实现，保持行为语义一致。
"""
...
class Memory5Store:
    ...
    def add(self, memory: Dict) -> str:
        ...
        self._flush()
        logger.info(f"Memory5Store 添加记忆 {memory['id']} (layer={memory.get('layer')})")
        return memory["id"]
    ...
    def update(self, memory_id: str, **kwargs) -> bool:
        ...
        for k, v in kwargs.items():
            ...
            mem[k] = v
        self._flush()
        logger.info(f"Memory5Store 更新记忆 {memory_id}")
        return True
    ...
    def delete(self, memory_id: str) -> bool:
        for i, mem in enumerate(self._memories):
            if mem["id"] == memory_id:
                ...
                self._flush()
                logger.info(f"Memory5Store 删除记忆 {memory_id}")
                return True
        return False
    ...
    def delete_batch(self, memory_ids: List[str]) -> int:
        ...
        if deleted > 0:
            self._flush()
            logger.info(f"Memory5Store 批量删除 {deleted} 条记忆")
        return deleted
    ...
    def _flush(self):
        ...
        try:
            ...
            logger.error(f"Memory5Store 写入失败: {e}")
            if temp_path.exists():
    ...
    def _load(self):
        ...
        for item in raw:
            try:
                ...
                logger.warning(f"Memory5Store 跳过损坏条目: {e}")
        logger.info(f"Memory5Store 加载完成: {len(self._memories)} 条记忆")
    ...
    def _read_with_recovery(self) -> list:
        try:
            ...
            logger.warning(f"Memory5Store 文件损坏或不存在: {e}")
            backups = sorted(
            ...
            for bak_path in backups:
                try:
                    ...
                    if isinstance(raw, list):
                        logger.info(f"Memory5Store 从备份 {bak_path} 恢复成功")
                        return raw
                except Exception:
                    continue
            logger.warning("Memory5Store 所有备份恢复失败，初始化为空列表")
            return []
    ...
    def _validate_item(self, item: Dict) -> Dict:
        ...
        if not isinstance(validated.get("id"), str) or not validated["id"].strip():
            ...
            logger.warning(f"Memory5Store 缺少有效 id，已生成: {validated['id']}")
    ...
    def clear_all(self):
        ...
        for bak_path in self._storage_dir.glob("memories.json.bak.*"):
            try:
                bak_path.unlink()
                logger.info(f"Memory5Store 已删除备份: {bak_path}")
            except OSError as e:
                logger.warning(f"Memory5Store 删除备份失败 {bak_path}: {e}")
